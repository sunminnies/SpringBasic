...
import sunmin.spring.beans.HelloSpring5Bean04Factory;

public class HelloSpring5App05 {
	
	// 스프링 프레임워크 첫번째 사례
    // 이전 예제에서는 factory 패턴을 이용해서 객체를 생성하는 코드를 작성했었음
    // 하지만, 스프링을 사용하면 이러한 과정은 필요X
	
	// 스프링 프레임워크
    // DI, IoC, AOP를 적용한 콘솔/GUI/웹 응용프로그램 개발에 사용하는 오픈소스 라이브러리
    ...
    // 스프링이 대신 객체를 생성하고 프로그램에서는 DI를 통해 이것을 사용함

    // bean05.xml에 스프링 컨테이너가 미리 생성해두어야 할 객체와 이름을 정의해 둠
    // 프로그램이 실행되면, bean05.xml에 작성한 객체정보를
    // 스프링 컨테이너가 읽어서 객체를 만들어 둠
    // BeanFactry에서 getBean이라는 메서드로
    // 해당 객체를 가져와서 HelloSpring5Bean03 변수에 주입
    // 이 변수의 sayHello 메서드를 호출하고 인삿말 출력
	
	// 스프링 컨테이너
    // 스프링 기반 프로그램은 일반적으로 스프링 컨테이너를 통해 객체가 생성되고 소멸함
	...
	public static void main(String[] args) {
		BeanFactory bf = new XmlBeanFactory(new FileSystemResource("src/bean05.xml"));
